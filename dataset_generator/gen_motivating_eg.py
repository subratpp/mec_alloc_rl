...
def run_mobilenet(num_of_users, ram):
    ...
    for i in range(num_of_execution):
        begin = time.time() 
        try:
            os.popen("cd mobilenet2; sudo systemd-run --scope -p MemoryMax={}M python3 mobilenet.py {}".format(ram, num_of_users)).read()
        except:
            return 0
        end = time.time()
        total_time += end - begin
...
if system_check!=1:
    ...
    try:
        print(">>>>Testing YOLO")
        run_time = run_yolo(500, 2000) #RAM in MB
    ...
    try:
        ...
        run_time = run_mobilenet(500, 3000) #RAM in MB
    ...
    y_t, m_t = run_services(400, 400, 3000)
    ...
    try:
        ...
        exe_time_y, exe_time_m, wk_g = services_execution_time(500, 500, 3, 4000, 40, 5000)
...
for epoch in range(epochs):
    for users_yolo in range(1, 6, 1):                        
        for users_mnet in range(1, 6, 1):
